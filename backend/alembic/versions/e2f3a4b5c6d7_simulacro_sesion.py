...
def upgrade() -> None:
    op.execute("""
        CREATE TYPE estado_simulacro AS ENUM ('activo', 'pausado', 'entre_bloques', 'completado')
    """)
    op.create_table(
        'simulacro_sesiones',
        sa.Column('id', sa.Integer(), primary_key=True, index=True),
        sa.Column('usuario_id', sa.Integer(), sa.ForeignKey('usuarios.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('estado', sa.Enum('activo', 'pausado', 'entre_bloques', 'completado', name='estado_simulacro'), nullable=False, server_default='activo'),
        ...
        sa.Column('preguntas_ids', sa.Text(), nullable=False, server_default='[]'),
        sa.Column('respuestas', sa.Text(), nullable=False, server_default='{}'),
