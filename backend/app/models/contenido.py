...
class Materia(Base):
    ...
    preguntas: Mapped[list["Pregunta"]] = relationship(
        "Pregunta", back_populates="materia"
    )
    ...
    estadisticas: Mapped[list["EstadisticaUsuario"]] = relationship(
        "EstadisticaUsuario", back_populates="materia"
    )
...
class Pregunta(Base):
    ...
    tipo: Mapped[TipoPregunta] = mapped_column(
        Enum(TipoPregunta, name="tipo_pregunta"), nullable=False
    )
    nivel_dificultad: Mapped[NivelDificultad] = mapped_column(
        Enum(NivelDificultad, name="nivel_dificultad"),
        nullable=False,
    ...
    pistas: Mapped[list["Pista"]] = relationship(
        "Pista", back_populates="pregunta", cascade="all, delete-orphan"
    )
    reportes: Mapped[list["Reporte"]] = relationship(
        "Reporte", back_populates="pregunta"
    )
    respuestas_usuarios: Mapped[list["RespuestaUsuario"]] = relationship(
        "RespuestaUsuario", back_populates="pregunta"
    )
...
class Respuesta(Base):
    ...
    Soporta texto e imagen (imagen_url) — pueden coexistir o ser solo uno.
    """
    ...
    pregunta_id: Mapped[int] = mapped_column(
        ForeignKey("preguntas.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    respuestas_usuario: Mapped[list["RespuestaUsuario"]] = relationship(
        "RespuestaUsuario", back_populates="opcion_elegida"
    )
