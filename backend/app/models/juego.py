...
class SesionJuego(Base):
    ...
    modo_juego:       Mapped[ModoJuego] = mapped_column(Enum(ModoJuego, name="modo_juego"), nullable=False)
    ...
    # Relaciones
    usuario:    Mapped["Usuario"]               = relationship("Usuario", back_populates="sesiones", foreign_keys=[usuario_id])
    respuestas: Mapped[list["RespuestaUsuario"]] = relationship("RespuestaUsuario", back_populates="sesion", cascade="all, delete-orphan")
...
class RespuestaUsuario(Base):
    ...
    sesion_id:        Mapped[int] = mapped_column(ForeignKey("sesiones_juego.id", ondelete="CASCADE"), nullable=False, index=True)
    pregunta_id:      Mapped[int] = mapped_column(ForeignKey("preguntas.id"), nullable=False, index=True)
    opcion_elegida_id: Mapped[int | None] = mapped_column(ForeignKey("opciones_respuesta.id"), nullable=True)
    ...
    # Relaciones
    sesion:        Mapped["SesionJuego"]   = relationship("SesionJuego", back_populates="respuestas")
    pregunta:      Mapped["Pregunta"]      = relationship("Pregunta", back_populates="respuestas_usuarios")
    opcion_elegida: Mapped["Respuesta | None"] = relationship("Respuesta", back_populates="respuestas_usuario")
...
class EstadisticaUsuario(Base):
    ...
    total_correctas:   Mapped[int]   = mapped_column(Integer, default=0, nullable=False)
    porcentaje_acierto: Mapped[float] = mapped_column(Integer, default=0, nullable=False)
...
class PartidaOnline(Base):
    ...
    jugador2_id: Mapped[int | None] = mapped_column(ForeignKey("usuarios.id"), nullable=True)   # NULL mientras busca oponente
    sesion_j1_id: Mapped[int | None] = mapped_column(ForeignKey("sesiones_juego.id"), nullable=True)
    sesion_j2_id: Mapped[int | None] = mapped_column(ForeignKey("sesiones_juego.id"), nullable=True)
    ganador_id:  Mapped[int | None] = mapped_column(ForeignKey("usuarios.id"), nullable=True)

    modo:   Mapped[ModoPartida]  = mapped_column(Enum(ModoPartida,  name="modo_partida"),  nullable=False)
    estado: Mapped[EstadoPartida] = mapped_column(Enum(EstadoPartida, name="estado_partida"), nullable=False, default=EstadoPartida.buscando)
    ...
    jugador2: Mapped["Usuario | None"] = relationship("Usuario", foreign_keys=[jugador2_id])
    ganador:  Mapped["Usuario | None"] = relationship("Usuario", foreign_keys=[ganador_id])
    poderes:  Mapped[list["PoderPartida"]] = relationship("PoderPartida", back_populates="partida", cascade="all, delete-orphan")
...
class PoderPartida(Base):
    ...
    id:          Mapped[int] = mapped_column(Integer, primary_key=True)
    partida_id:  Mapped[int] = mapped_column(ForeignKey("partidas_online.id", ondelete="CASCADE"), nullable=False)
    ...
    partida: Mapped["PartidaOnline"] = relationship("PartidaOnline", back_populates="poderes")
