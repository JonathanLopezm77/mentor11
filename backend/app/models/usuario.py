...
class Usuario(Base):
    ...
    avatar: Mapped["Avatar"] = relationship(
        "Avatar", back_populates="usuario", uselist=False, cascade="all, delete-orphan"
    )
    suscripciones: Mapped[list["Suscripcion"]] = relationship(
        "Suscripcion", back_populates="usuario", cascade="all, delete-orphan"
    )
    tokens_rec: Mapped[list["TokenRecuperacion"]] = relationship(
        "TokenRecuperacion", back_populates="usuario", cascade="all, delete-orphan"
    )
    sesiones: Mapped[list["SesionJuego"]] = relationship(
        "SesionJuego", back_populates="usuario", foreign_keys="SesionJuego.usuario_id"
    )
    estadisticas: Mapped[list["EstadisticaUsuario"]] = relationship(
        "EstadisticaUsuario", back_populates="usuario", cascade="all, delete-orphan"
    )
    aulas_creadas: Mapped[list["Aula"]] = relationship(
        "Aula", back_populates="profesor", foreign_keys="Aula.profesor_id"
    )
    aulas_inscrito: Mapped[list["AulaEstudiante"]] = relationship(
        "AulaEstudiante",
        back_populates="estudiante",
        foreign_keys="AulaEstudiante.estudiante_id",
    )
    reportes: Mapped[list["Reporte"]] = relationship(
        "Reporte", back_populates="usuario", foreign_keys="Reporte.usuario_id"
    )
    notificaciones: Mapped[list["Notificacion"]] = relationship(
        "Notificacion", back_populates="usuario", cascade="all, delete-orphan"
    )
    logros_obtenidos: Mapped[list["UsuarioLogro"]] = relationship(
        "UsuarioLogro", back_populates="usuario", cascade="all, delete-orphan"
    )
    logs: Mapped[list["LogActividad"]] = relationship(
        "LogActividad", back_populates="usuario", foreign_keys="LogActividad.usuario_id"
    )
...
class Suscripcion(Base):
    ...
    plan: Mapped[PlanSuscripcion] = mapped_column(
        Enum(PlanSuscripcion, name="plan_suscripcion"), nullable=False
    )
